...
class CmdStop(Command):
    """
    Stop your current combat queue and break any active loops.
    ...
    def func(self):
        caller = self.caller
        # Clear the queue and disable the loop
        caller.db.action_sequence = []
        caller.db.original_sequence = []
        caller.db.action_loop = False
        caller.db.current_target = None
        
        caller.msg("|gYou have stopped your combat sequence.|n")

class CmdQueue(Command):
    """
    Queue a sequence of combat actions.
    
    Usage:
        ;action1,action2,...:target
    ...
    def func(self):
        # Use self.raw_string instead of self.raw
        raw_input = self.raw_string.strip()
        
        # Remove the leading ';'
        content = raw_input[1:].strip()
        
        if not content or ":" not in content:
            self.msg("Usage: ;<actions>:<target> (e.g., ;smash,bash:me)")
            return
            
        action_str, target_name = content.split(":", 1)
        target = self.caller.search(target_name.strip())
        
        if target:
            self.caller.queue_action(action_str.strip(), target)